...
this.movementEnabled = true; 
    app.on('movement:disable', function () {
        this.movementEnabled = false;
        this.releasePointerLock(); // Release pointer lock when disabling movement
    }, this);
    app.on('movement:enable', function () {
        this.movementEnabled = true;
        this.requestPointerLock(); // Request pointer lock when enabling movement
    }, this);

    app.on('firstperson:forward', function (value) {
        if (this.movementEnabled) this.forward = value;
    }, this);
    app.on('firstperson:strafe', function (value) {
        if (this.movementEnabled) this.strafe = value;
    }, this);
    ...
    app.on('firstperson:jump', function () {
        if (this.movementEnabled) this.jump = true;
    }, this);

};

FirstPersonView.prototype.releasePointerLock = function() {
...
FirstPersonView.prototype.requestPointerLock = function() {
...
KeyboardInput.prototype.initialize = function() {
    ...
    // Listen for custom events to enable/disable movement
    app.on('movement:disable', () => {
        this.movementEnabled = false;
    });

    app.on('movement:enable', () => {
        this.movementEnabled = true;
    });
};
...
TouchInput.prototype.initialize = function() {
    ...
    // Detect and adjust the resolulution
    if (isMobile()) {
